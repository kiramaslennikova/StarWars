...
fetch("data_wrangling/data/ratings_distribution_by_genre.json")
    .then(response => response.json())
    .then(data => {
        ...
        function updateChart(genre) {
            ...
            const imdbData = genreData.filter(item => item.rating_type === "IMDb");
            const metascoreData = genreData.filter(item => item.rating_type === "Metascore");

            const labels = [...new Set([...imdbData.map(d => d.score), ...metascoreData.map(d => d.score)])].sort((a, b) => a - b);

            const imdbCounts = labels.map(score => {
                const match = imdbData.find(d => d.score === score);
                return match ? match.count : 0;
            });

            const metascoreCounts = labels.map(score => {
                const match = metascoreData.find(d => d.score === score);
                return match ? match.count : 0;
            });
